...
import utils.Util;
import static utils.Util.getLabelsetsByValues;
...
public class RunApp extends javax.swing.JFrame {
    ...
    MultiLabelInstances dataset_train, dataset_test;    
    ArrayList<Instances> transformedDatasets = new ArrayList<Instances>();
    ArrayList<MultiLabelInstances> listDatasetsTrain, listDatasetsTest;
             
    //Charts
    ChartPanel labelFrequencyChart, labelsHistogramChart, labelsetFrequencyChart;
    ChartPanel labelsBoxChart, attributesBoxChart;
    ChartPanel IRLabelsetChart, IRInterClassChart, IRIntraClassChart;

    //Selecting metrics buttons
    JButton buttonAll, buttonNone, buttonInvert, buttonCalculate, buttonSave, buttonClear;
    JButton exportChiPhi, exportCoocurrenceTable, exportHeatmapTable, exportCoocurrenceGraph, exportHeatmapGraph;
    
    //Tables
    JTable jTablePrincipal, jTableMulti;
    JTable jTableChiPhi, jTableCoocurrence, jTableHeatmap, fixedTableChiPhi, fixedTableCoocurrence, fixedTableHeatmap;
    
    //Table models
    TableModel tmBox, tmLabelFrequency, tmLabelsetFrequency, tmLabelsHistogram;
    TableModel tmAttributes, tmCoocurrences, tmHeatmap, tmIRLabelset, tmIRInterClass, tmIRIntraClass;
    
    //Multiple datasets
    DefaultListModel multipleDatasetList = new DefaultListModel();
    ArrayList<MultiLabelInstances> listMultiDatasets;    
    ArrayList<Boolean> isMekaList;
    ArrayList<String> datasetNames;
    
    
    ImbalancedFeature[] sortedLabelsets = null;
    ImbalancedFeature[] IRSortedLabelsets = null;
    ImbalancedFeature[] frequencySortedLabels;
    ...
    int[] idLabels_IRInterClass = null;     
    double[] id_IRInterClass = null;  
    ContainerIRInterClass IRFreq=null;
    double[] IRInterClass = null;
    double[] IRIntraClass = null;
    ImbalancedFeature[] labelFrequency = null;
    double[] labelsetFrequency = null;
    ...
    ArrayList<String> labelsetStringsFreq;
    ArrayList<String> labelsetStringsIR;   
    ...
    public RunApp() 
    {
        ...
        fixedTableCoocurrence = new JTable();
        fixedTableHeatmap = new JTable();
        jTableCoocurrence= new JTable();
        ...
        startConfigMultipleDatasets();
    }
    
    
    private void initChiPhiTable()
    {
        ...
        fixedTableCoocurrence.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        jTableCoocurrence.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        fixedTableCoocurrence.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        jTableCoocurrence.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
    ...
    private void initConfig()
    {
        ...
        */
        ...
        RunApp.this.createTableMetricsPrincipal(jTablePrincipal,panelSummary,buttonAll,buttonNone,buttonInvert,buttonCalculate,buttonSave, buttonClear, 30,190,780,280); //tab Database //35,155,500,355

        jTableMulti = setMetricsHelp(jTableMulti);
        RunApp.this.createTableMetricsMulti(jTableMulti,jPanelMulti,buttonAll,buttonNone,buttonInvert,buttonCalculate,buttonSave, 25,15,510,420); //tab Multi
         ...
         * Preprocess
         */
        //Add radio buttons to group and set tooltips
        ...
        labelsetFrequencyChart = createJChart(panelExamplesPerLabelset, "bar","Frequency","Labelsets",false, "Labelset frequency");
        labelsHistogramChart = createJChart(panelLabelsPerExample,"bar", "Frequency","Number of labels",false, "Labels histogram");
        labelsBoxChart = createGraph(panelBoxDiagram);
        attributesBoxChart = createGraph(panelBoxDiagramAtt);
        
        IRInterClassChart = createJChart(panelIRperLabelInterClass, "bar", "IR inter-class","Labels",false, "IR per label inter class");
        IRIntraClassChart = createJChart(panelIRperLabelIntraClass, "bar", "IR intra-class","Labels",false, "IR per label intra class");

        IRLabelsetChart = createJChart(panelIRperLabelset, "bar", "IR","Labelsets",false, "IR per labelset");
      
        jTableChiPhi = setChiPhiTableHelp(jTableChiPhi);
        jTableCoocurrence = setCoocurrenceTableHelp(jTableCoocurrence);
        ...
        jTableCoocurrence.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        JScrollPane scrollPane = new JScrollPane(jTableCoocurrence, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        
        scrollPane.setBounds(20, 20, 780, 390);
        jTableCoocurrence.setBorder(BorderFactory.createLineBorder(Color.black));
        panelCoOcurrenceValues.add(scrollPane, BorderLayout.CENTER);
      
        //Configure jTable heapmap VALUES
        jTableHeatmap.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        ...
        //Create dependences buttons
        createButtonExportDependencesTable(jTableChiPhi,fixedTableChiPhi ,panelChiPhi ,exportChiPhi,710,415, "ChiPhi"); // chiLabel and PhiLabel values
        createButtonExportDependencesTable(jTableCoocurrence,fixedTableCoocurrence,panelCoOcurrenceValues ,exportCoocurrenceTable,710,415, "Coocurrence");//graph values
        ...
        createButtonExportDependencesGraph(panelCoOcurrence ,exportCoocurrenceGraph,720,440);
        createButtonExportDependencesGraph(panelHeatmapGraph,exportHeatmapGraph,720,440);
        Border border = BorderFactory.createLineBorder(Color.gray, 1);

        panelHeatmap.setBorder(border); 
        jTableChiPhi.setBorder(border);
        jTableCoocurrence.setBorder(border);
    ...
    private void createButtonExportDependencesTable(final JTable jtable, final JTable columns,
            JPanel jpanel, JButton jButtonExport, int posx, int posy, final String table)
    {
        //button export table
    ...
    private void createButtonExportDependencesGraph(final JPanel jpanel, 
    ...
    private void createTableMetricsPrincipal(final JTable jtable, JPanel jpanel, 
            JButton button_all, JButton button_none, JButton button_invert, 
            JButton button_calculate,JButton button_save, JButton button_clear, 
            int posx,int posy, int width, int height)
    {
        createTableMetricsPrincipal(jtable,jpanel, Util.getRowData(),posx,posy,width,height);        
        
        //button ALL
        button_all = new JButton("All");
        button_all.setBounds(posx, posy+height+5, 80, 20);
        ...
        button_none.setBounds(posx+90, posy+height+5, 80, 20);
        ...
        button_invert.setBounds(posx+180, posy+height+5, 80, 20);
        ...
        button_clear.setBounds(posx+270, posy+height+5, 80, 20);
        ...
        button_calculate.setBounds(posx+590, posy+height+5, 95, 25);
        ...
        button_save.setBounds(posx+695, posy+height+5, 80, 25);
    ...
    private void createTableMetricsMulti(final JTable jtable, JPanel jpanel, 
            JButton button_all, JButton button_none, JButton button_invert, 
            JButton button_calculate, JButton button_save, int posx, int posy, 
            int width, int heigh)
    {
        createTableMetricsMulti(jtable, jpanel, Util.getRowDataMulti(), posx, posy, width, heigh);  
    ...
    private void buttonShowMostRelatedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonShowMostRelatedActionPerformed
        ...
        ArrayList<AttributesPair> pares_seleccionados =  Util.getSelectedLabelPairs(pairs, seleccionados);

        String[] labelname=Util.StringListToArray(seleccionados);
    ...
    private void tabsImbalanceStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_tabsImbalanceStateChanged
        ...
        if(tmLabelFrequency !=null && tmLabelsetFrequency!=null){
            ...
            if (tabsImbalance.getSelectedIndex()==0)
            {
            ...
            else if(tabsImbalance.getSelectedIndex()==1)
            {
    ...
    private void tableImbalanceMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableImbalanceMouseClicked
        ...
        {
            int selectedRow = tableImbalance.getSelectedRow();

            if(sortedLabelsets == null){
                return;
            }

            ArrayList<String> labelNames = Util.getLabelNamesFromLabelset(dataset, sortedLabelsets[selectedRow].getName());

            String[] args = new String[2];

            args[0]= sortedLabelsets[selectedRow].getName();
            args[1]= Integer.toString(sortedLabelsets[selectedRow].getAppearances());

            int posx = this.getBounds().x;
            int posy = this.getBounds().y;

            EmergentOutput mo = new EmergentOutput(dataset, posx, posy+50,args,labelNames,frequencySortedLabels, isMeka);
        ...
        else if(tabsImbalance.getSelectedIndex()==5)
        {
            ...
            if(selectedRow == 0){
                ...
                double [] labelFrequencies = Util.getLabelFrequency(labelFrequency);

                HeapSort.sort(labelFrequencies);

                Util.updateXYValues(labelsBoxChart, HeapSort.get_array_sorted());
            }
            else if(selectedRow == 1){
                tableImbalance.clearSelection();

                if(labelsetFrequency == null) {
                    return;
                }

                HeapSort.sort(labelsetFrequency);

                labelsBoxChart.getChart().setTitle("# Examples per Labelset");
                labelsBoxChart.getChart().getXYPlot().clearAnnotations();
        ...
        else if(tabsImbalance.getSelectedIndex()==6)
        {
            ...
            if(IRSortedLabelsets == null) {
                return;
            }

            ArrayList<String> labelNames = Util.getLabelNamesFromLabelset(dataset, IRSortedLabelsets[selectedRow].getName());

            String[] args = new String[2];

            args[0] = IRSortedLabelsets[selectedRow].getName();
            args[1] = Integer.toString(IRSortedLabelsets[selectedRow].getAppearances());
        ...
        try{
            ...
            */
            ...
            if(!(radioNoFS.isSelected() && radioNoIS.isSelected() && radioNoSplit.isSelected())){
                if((listDatasetsTrain.isEmpty() && listDatasetsTest.isEmpty()) && (radioRandomCV.isSelected() || radioIterativeStratifiedCV.isSelected() || radioLPStratifiedCV.isSelected())){
                    JOptionPane.showMessageDialog(null, "You must click on Start before.", "alert", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION)
            {
                ...
                if(fc.isDirectorySelectionEnabled())
                {
                    ...
                    if(radioNoFS.isSelected() && radioNoIS.isSelected() && radioNoSplit.isSelected())
                    {
                        ...
                        try {
                            ...
                            if(format.toLowerCase().contains("meka")){
                                ...
                                Util.saveMekaDataset(wr_train, dataset, dataset.getDataSet().relationName());
                                ...
                                Util.saveDataset(wr_train, dataset, dataset.getDataSet().relationName());
                            ...
                            && radioNoSplit.isSelected())//Feature and/or instance selection
                    {
                        BufferedWriter bw_train = null;
                        try {
                            String name = datasetName.substring(0,datasetName.length()-5);

                            if(format.toLowerCase().contains("meka")){
                                String dataPath = file.getAbsolutePath()+"/"+name+ preprocessedType + ".arff";

                                bw_train = new BufferedWriter(new FileWriter(dataPath));
                                PrintWriter wr_train = new PrintWriter(bw_train);

                                if(radioNoFS.isSelected()){
                                    Util.saveMekaDataset(wr_train, preprocessedDataset, preprocessedDataset.getDataSet().relationName() + preprocessedType);
                                }
                                else{
                                    Util.saveMekaDataset(wr_train, preprocessedDataset, name + preprocessedType);
                                }
                                ...
                                String dataPath = file.getAbsolutePath()+"/"+name+ preprocessedType + ".arff";
                                path_xml = file.getAbsolutePath()+"/"+name+ preprocessedType +".xml";

                                bw_train = new BufferedWriter(new FileWriter(dataPath));
                                PrintWriter wr_train = new PrintWriter(bw_train);
                                
                                if(radioNoFS.isSelected()){
                                    Util.saveDataset(wr_train, preprocessedDataset, preprocessedDataset.getDataSet().relationName() + preprocessedType);
                                }
                                else{
                                    Util.saveDataset(wr_train, preprocessedDataset, name+ preprocessedType);
                                }
                                ...
                                Util.saveXML(wr_xml,preprocessedDataset);
                    ...
                    if(radioIterativeStratifiedHoldout.isSelected()|| radioRandomHoldout.isSelected() || radioLPStratifiedHoldout.isSelected()) //holdout
                    {
                        ...
                        try {
                            ...
                            if(format.toLowerCase().contains("meka")){
                                ...
                                if(radioNoFS.isSelected()){
                                    Util.saveMekaDataset(wr_train, dataset_train, dataset_train.getDataSet().relationName());
                                }
                                else{
                                    Util.saveMekaDataset(wr_train, dataset_train, name_dataset);
                                }
                                ...
                                if(radioNoFS.isSelected()){
                                    Util.saveMekaDataset(wr_test, dataset_test, dataset_test.getDataSet().relationName());
                                }
                                else{
                                    Util.saveMekaDataset(wr_test, dataset_test, name_dataset);
                                }
                                ...
                                if(radioNoFS.isSelected()){
                                    Util.saveDataset(wr_train, dataset_train, dataset_train.getDataSet().relationName());
                                }
                                else{
                                    Util.saveDataset(wr_train, dataset_train, name_dataset);
                                }
                                ...
                                if(radioNoFS.isSelected()){
                                    Util.saveDataset(wr_test, dataset_test, dataset_test.getDataSet().relationName());
                                }
                                else{
                                    Util.saveDataset(wr_test, dataset_test, name_dataset);
                                }
                                ...
                                Util.saveXML(wr_xml, dataset_train);
                    ...
                    else if(radioIterativeStratifiedCV.isSelected()|| radioRandomCV.isSelected() || radioLPStratifiedCV.isSelected())//CROSS VALIDATION
                    {
                        try{
                            if(format.toLowerCase().contains("meka")){
                                if(radioNoFS.isSelected() && radioNoIS.isSelected()){
                                    Util.saveMekaDataset(listDatasetsTrain,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5), "-train");
                                    Util.saveMekaDataset(listDatasetsTest,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5), "-test");
                                }
                                if(radioNoFS.isSelected()){
                                    Util.saveMekaDataset(listDatasetsTrain,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-train");
                                    Util.saveMekaDataset(listDatasetsTest,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-test");
                                }
                                else{
                                    Util.saveMekaDatasetNoViewes(listDatasetsTrain,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-train");
                                    Util.saveMekaDatasetNoViewes(listDatasetsTest,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-test");
                                }
                            }
                            else{
                                if(radioNoFS.isSelected() && radioNoIS.isSelected()){
                                    Util.saveDataset(listDatasetsTrain,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5), "-train");
                                    Util.saveDataset(listDatasetsTest,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5), "-test");
                                    path_xml = file.getAbsolutePath()+"/"+datasetName.substring(0,datasetName.length()-5)+".xml";
                                }
                                else if(radioNoFS.isSelected()){
                                    Util.saveDataset(listDatasetsTrain,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-train");
                                    Util.saveDataset(listDatasetsTest,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-test");
                                    path_xml = file.getAbsolutePath()+"/"+datasetName.substring(0,datasetName.length()-5)+ preprocessedType + ".xml";
                                }
                                else{
                                    Util.saveDatasetNoViews(listDatasetsTrain,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-train");
                                    Util.saveDatasetNoViews(listDatasetsTest,file.getAbsolutePath(), datasetName.substring(0,datasetName.length()-5),  preprocessedType + "-test");
                                    path_xml = file.getAbsolutePath()+"/"+datasetName.substring(0,datasetName.length()-5)+ preprocessedType + ".xml";
                                }
                                ...
                                Util.saveXML(wr_xml,listDatasetsTrain.get(0));
    ...
    private void textChooseFileKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_textChooseFileKeyPressed
        if(evt.getKeyCode() == KeyEvent.VK_ENTER)
        {
            ...
            filename_database_xml = Util.getXMLString(filename_database_arff);
            filename_database_xml = Util.getXMLFilename(filename_database_xml);
            loadDataset(filename_database_arff, filename_database_xml);
        }
    ...
    private void tableAttributesLeftMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableAttributesLeftMouseClicked
        ...
        {
            int selectedRow = tableAttributesLeft.getSelectedRow();
            String attributeName = tableAttributesLeft.getValueAt(selectedRow, 0).toString();
            Instances instances = dataset.getDataSet();

            Attribute currentAttribute = instances.attribute(attributeName);

            double[] attributeValues = instances.attributeToDoubleArray(currentAttribute.index());

            HeapSort.sort(attributeValues);

            attributesBoxChart.getChart().getXYPlot().clearAnnotations();
            
            Util.updateXYValues(attributesBoxChart, HeapSort.get_array_sorted());
        }
    ...
    private void buttonShowMostFrequentURelatedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonShowMostFrequentURelatedActionPerformed
        ...
        ArrayList<String> listSelectedsFreq = new  ArrayList();
        ...
        ArrayList<String> listSelectedRelated = selectTopCoocurrenceLabels(n, true);
        for(int i=0; i<n; i++){
            if(! listSelectedsFreq.contains(listSelectedRelated.get(i))){
                listSelectedsFreq.add(listSelectedRelated.get(i));
            }
        }
        tableCoOcurrenceLeft.addRowSelectionInterval(0, n-1);

        ArrayList<AttributesPair> selectedPairs = Util.getSelectedLabelPairs(pairs, listSelectedsFreq);

        String[] labelName = Util.StringListToArray(listSelectedsFreq);
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION)
        {
            ...
            {  
                try
                {
                    ...
                    Util.saveMVTableCSV(wr, jTable2, views, dataset);
                ...
                if(fc.isDirectorySelectionEnabled())
                {
                    ...
                    try {
                        ...
                        if(format.toLowerCase().contains("meka")){
                            ...
                            Util.saveMekaDatasetMV(wr_train, mvData, name_dataset, viewsString);
                            ...
                            String dataPath = file.getAbsolutePath()+"/"+name_dataset+ preprocessedType + ".arff";
                            pathXML = file.getAbsolutePath()+"/"+name_dataset+ preprocessedType +".xml";

                            bw_train = new BufferedWriter(new FileWriter(dataPath));
                            PrintWriter wr_train = new PrintWriter(bw_train);

                            Util.saveDatasetMV(wr_train, mvData, name_dataset, viewsString);

                            wr_train.close();
                            bw_train.close();

                            BufferedWriter bw_xml = new BufferedWriter(new FileWriter(pathXML));
                            PrintWriter wr_xml = new PrintWriter(bw_xml);

                            Util.saveXML(wr_xml, mvData);
        ...
        if (returnVal == JFileChooser.OPEN_DIALOG)
        {
            ...
            for(File f1 : files){
                ...
                try
                {
                    ...
                    isMeka = Util.isMeka(line);
                    isMekaList.add(isMeka);

                    if(isMeka)
                    {
                        int labelCount = Util.getNumLabelsFromArff(line);                

                        if(labelCount > 0){
                            labelNamesFound = new String[labelCount];

                            while(labelsFound < labelCount)
                            {
                                line = bf.readLine();
                                labelName = Util.getLabelName(line);

                                if(labelName!= null)
                                {
                                    labelNamesFound[labelsFound]=labelName;
                                    labelsFound++;
                                }
                            }
                        }
                            ...
                            while(!(line = bf.readLine()).contains("@data")){
                                if(!line.trim().equals("")){
                                    for(int s=0; s<labelCount-1; s++){
                                        sStrings[s] = sStrings[s+1];
                                    }
                                    sStrings[labelCount-1] = line;
                                }
                            }

                            for(int i=0; i<labelCount; i++){
                                labelName = Util.getLabelName(sStrings[i]);
                        ...
                        Util.writeMulanXML(wr_xml, labelNamesFound);

                        bw_xml.close();
                        wr_xml.close();

                        filename_database_xml = Util.getXMLFilename(filename_database_xml_path1);
                    }
                    else
                    {
                        isMeka= false;
                ...
                try{
                    ...
                    listMultiDatasets.add(current);
                    datasetNames.add(name);
                    multipleDatasetList.addElement(name );
                }
    ...
    private int preprocess(){
        listDatasetsTrain = new ArrayList();
        ...
        if(!radioNoSplit.isSelected()){
            ...
            //Random CV
                ...
                try{
                    ...
                    Instances datasetTemp = temp.getDataSet();

                    int seed = (int)(Math.random()*100)+100;
                    Random rand = new Random(seed);

                    datasetTemp.randomize(rand);
                    ...
                    for(int i=0; i<datasetTemp.numInstances(); i++){
                        foldsCV[i%nFolds].add(datasetTemp.get(i));
                    }
                    ...
                    for(int i=0; i<nFolds; i++){
                        ...
                        listDatasetsTrain.add(new MultiLabelInstances(train, preprocessDataset.getLabelsMetaData()));
                        listDatasetsTest.add(new MultiLabelInstances(test, preprocessDataset.getLabelsMetaData()));
                    }
                }
            ...
            //Iterative stratified holdout
                ...
                double percetnageSplit = Double.parseDouble(split);
                if((percetnageSplit <= 0) || (percetnageSplit >= 100)){
                    JOptionPane.showMessageDialog(null, "The percentage must be a number in the range (0, 100).", "alert", JOptionPane.ERROR_MESSAGE);
                    return -1;
                }
                ...
                try{
                    ...
                    MultiLabelInstances [] partitions = pre.split(preprocessDataset, percetnageSplit);
            ...
            //LP stratified holdout
                ...
                double percentageSplit = Double.parseDouble(split);
                ...
                try{
                    IterativeTrainTest pre = new IterativeTrainTest();
                    MultiLabelInstances [] partitions = pre.split(preprocessDataset, percentageSplit);
    ...
    private int loadDataset(int returnVal, JFileChooser jfile1, boolean deleteXML){
        if (returnVal == JFileChooser.OPEN_DIALOG)
        {
            ...
            filename_database_xml = Util.getXMLFilename(filename_database_xml_path);
            ...
            try
            {
                ...
                if(line.contains("-V:")){
                    ...
                    String viewsLine = line.split("'")[1];
                    viewsLine = viewsLine.split("-V:")[1];
                    String [] intervals = viewsLine.split("!");
            ...
            if(!file_temp.exists())
            {
                filename_database_xml_path = Util.getXMLString(filename_database_arff);
                filename_database_xml = Util.getXMLFilename(filename_database_xml_path);
            }
            ...
            */
            ...
            if(f.exists() && !f.isDirectory()) { 
                ...
                loadDataset(filename_database_arff, null);
            }
    ...
    private int getLabelIndex(String name){
        for(int i=0; i<jTableCoocurrence.getColumnCount(); i++){
            if(jTableCoocurrence.getColumnName(i).equals(name)){
                return(i);
            }
        }
    ...
    private void showHeatMap(){        
        ...
        int[] selectedRows =tableHeatmapLeft.getSelectedRows();
        ...
        {
            listSelected .add((tableHeatmapLeft.getValueAt(selectedRows [i], 0).toString()));
            selectedIndex.add(getLabelIndex((tableHeatmapLeft.getValueAt(selectedRows [i], 0).toString())));
        }
    ...
    private void initTableMetrics(){
        ArrayList<String> metricsList = Util.getAllMetrics();
    ...
    private void saveCoocurrenceGraph() throws AWTException, IOException
    {
        ...
        FileNameExtensionFilter fname = new FileNameExtensionFilter(".png", "png");
    ...
    private void startConfigMultipleDatasets()
    {
        isMekaList = new ArrayList();
        listMultiDatasets = new ArrayList();
        datasetNames = new ArrayList();
        listMultipleDatasetsLeft.setModel(multipleDatasetList);
    }
    
    
    private mxGraphComponent createJGraphX(JPanel jpanel, ArrayList<AttributesPair> pairsList, 
            String[] labelNames, mxGraphComponent oldGraph )
    {
	...
	graph.getModel().beginUpdate();
        ...
        Object[] vertices = new Object[labelNames.length];
        ...
        try
        {
            ...
            {
                current = Util.getLabelByLabelname(labelNames[i],frequencySortedLabels);
                freq = current.getAppearances()/(dataset.getNumInstances()*1.0);

                strength =  Util.getBorderStrengthValue(min, max, numIntervals, freq);
                
                if(strength==1){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*6,20);
                }      
                else if (strength==2){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"ROUNDED;strokeWidth=2");
                }      
                else if (strength==3){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"ROUNDED;strokeWidth=3");
                }      
                else if (strength==4){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"ROUNDED;strokeWidth=4");
                }      
                else if (strength==5){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"strokeWidth=5");
                }      
                else if (strength==6){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"strokeWidth=6");
                }      
                else if (strength==7){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"strokeWidth=7");
                }      
                else if (strength==8){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"strokeWidth=8");
                }      
                else if (strength==9){
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"strokeWidth=9");
                }      
                else{
                    vertices[i] = graph.insertVertex(parent, null,labelNames[i], rand.nextInt(430), rand.nextInt(280), labelNames[i].length()*5,20,"strokeWidth=10");
                }       
            }
            ...
            if(!pairsList.isEmpty()){            
                ...
                for(int i=0;i<labelNames.length;i++)
                {
                    otherList = Util.getVertices(labelNames[i], pairsList);

                    for(String actual : otherList)
                    {
                        int index = Util.getIndex(labelNames, actual);

                        temp = Util.getAttributesPair(labelNames[i], actual, pairsList);
                        freq = temp.getAppearances()/(dataset.getNumInstances()*1.0);

                        strength = Util.getBorderStrengthValue(min, max, numIntervals, freq);

                        if(strength==1) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=1");
                        }
                        else if (strength==2) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=2");
                        }
                        else if (strength==3) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=3");
                        }
                        else if (strength==4) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=4");
                        }
                        else if (strength==5) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=5");
                        }
                        else if (strength==6) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=6");
                        }
                        else if (strength==7) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=7");
                        }
                        else if (strength==8) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=8");
                        }
                        else if (strength==9) {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=9");
                        }
                        else {
                            graph.insertEdge(parent, null,"",vertices[i], vertices[index], "startArrow=none;endArrow=none;strokeWidth=3");
                        }
	...
	mxGraphComponent newGraph = new mxGraphComponent(graph);               
        newGraph.getGraph().getModel().endUpdate();
        
        jpanel.setLayout(new BorderLayout());
        jpanel.setPreferredSize(new Dimension(550, 425));
        jpanel.add(newGraph,BorderLayout.CENTER);
    ...
    private HeatMap createHeatmapGraph(JPanel jpanel, double [][] coefficients, 
            ArrayList<AttributesPair> parisList, HeatMap oldGraph)
    {
        ...
        if((parisList != null) && (parisList.size() > 0)){
            HashSet<Integer> selected = new HashSet<Integer>();
            
            for(int i=0; i<parisList.size(); i++){
                selected.add(parisList.get(i).getIndex1());
                selected.add(parisList.get(i).getIndex2());
            }
    ...
    private void loadDataset(String filename_database_arff, String filename_database_xml)
    {
        try {        
            export2.setVisible(true);
            ...
            labelFrequency = Util.getFrequencyByLabel(dataset);
            labelFrequency = Util.orderImbalancedDataByFrequency(labelFrequency);
             
            imbalancedData = Util.getImbalancedByIRInterClass(dataset,labelFrequency);
             
            IRIntraClass = Util.getIRIntraClassValues(imbalancedData);
            HeapSort.sort(IRIntraClass);
            IRIntraClass = HeapSort.get_array_sorted();
            ...
            frequencySortedLabels = Util.getFrequencyByLabel(dataset);
            
            CategoryPlot temp1 = labelFrequencyChart.getChart().getCategoryPlot();
            temp1.clearRangeMarkers();
            Util.updateBarChartValues(frequencySortedLabels,dataset.getNumInstances(),temp1);

            HashMap<Integer,Integer> labelsPerExample = getLabelsetsByValues(stats);
                            
            Util.updateLineChartValues(dataset.getNumInstances(),labelsHistogramChart.getChart().getCategoryPlot(),labelsPerExample);
                   
            temp1 = labelsetFrequencyChart.getChart().getCategoryPlot();
            temp1.clearRangeMarkers();
            ...
            tmLabelsHistogram = jtable_lablelsxExamples(tableImbalance, labelsPerExample);
            ...
            temp1 = IRLabelsetChart.getChart().getCategoryPlot();
            temp1.clearRangeMarkers();
            tmIRLabelset = jchart_and_jtable_label_set_IR(tableImbalance,dataset,stats,IRLabelsetChart.getChart().getCategoryPlot());
            
            temp1= IRInterClassChart.getChart().getCategoryPlot();
            temp1.clearRangeMarkers();
            Util.updateIRChartValues(frequencySortedLabels, IRInterClass, temp1);
            
            temp1= IRIntraClassChart.getChart().getCategoryPlot();
            temp1.clearRangeMarkers();
            Util.updateIRChartValues(frequencySortedLabels, IRIntraClass, temp1);
            ...
            //tm_coocurrences   
            pairs = Util.getAttributePairs(dataset);   
             
            jtable_coefficient_values(dataset, pairs,"coocurrence");
            jTableCoocurrence.setDefaultRenderer(Object.class, new BaseRender("estandar",Double.MAX_VALUE));
            fixedTableCoocurrence.setDefaultRenderer(Object.class, new BaseRender("chi_fi_fixed",Double.MAX_VALUE));
            ...
            String[] labelname1=Util.StringListToArray(seleccionados);
            ...
            // jpanel8 box diagram
             labelsBoxChart.getChart().getXYPlot().clearAnnotations();
             labelsBoxChart.getChart().setTitle("");
             attributesBoxChart.getChart().getXYPlot().clearAnnotations();
             attributesBoxChart.getChart().setTitle("");
               
            DefaultXYDataset xyseriescollection = new DefaultXYDataset();
            DefaultXYDataset xyseriescollection1 = new DefaultXYDataset();
              
            labelsBoxChart.getChart().getXYPlot().setDataset(xyseriescollection);             
            labelsBoxChart.getChart().getXYPlot().setDataset(1, xyseriescollection1);
            
            attributesBoxChart.getChart().getXYPlot().setDataset(xyseriescollection);             
            attributesBoxChart.getChart().getXYPlot().setDataset(1, xyseriescollection1);
    ...
    public ArrayList<String> selectTopCoocurrenceLabels(int n, boolean selectInTable){
        ...
        for(int i=0; i<selectedLabels.size(); i++){
            s = jTableCoocurrence.getColumnName(selectedLabels.get(i));
    ...
    private void printMainMetrics(MultiLabelInstances dataset)
    {
        ...
        //Attributes
        //labelAttributesValue.setText(Util.getValueFormatted("Attributes", nAttributes));
        labelAttributesValue.setText(getMetricValueFormatted(attributes));
            
        //Labels
        //labelLabelsValue.setText(Util.getValueFormatted("Labels", Util.getMetricValue("Labels", dataset, isMeka)));
        labelLabelsValue.setText(getMetricValueFormatted(labels));    
        
        //Density
        //String density = Util.getMetricValue("Density", dataset, isMeka);
        ...
        //Cardinality
        //String cardinality = Util.getMetricValue("Cardinality", dataset, isMeka);
        ...
        //Diversity      
        //String diversity = Util.getMetricValue("Diversity", dataset, isMeka);
        ...
        //Bound
        //String bound = Util.getMetricValue("Bound", dataset, isMeka);
        //labelBoundValue.setText(Util.getValueFormatted("Bound", bound));
        labelBoundValue.setText(getMetricValueFormatted(bound));
                
        //Distinct labelset     
        //String distinct_labelset = Util.getMetricValue("Distinct labelsets", dataset, isMeka);
        ...
        //LxIxF
        //String LIF = Util.getMetricValue("Labels x instances x features", dataset, isMeka);
    ...
    private void button_saveActionPerformed_principal(java.awt.event.ActionEvent evt, JTable jtable) throws IOException
    {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION)
        {
            ...
            if(f1.getDescription().equals(".txt"))
            {
                ...
                Util.saveMetricsTXT(wr, metric_list, dataset, imbalancedData, isMeka, tableMetrics);
            ...
            else if(f1.getDescription().equals(".tex"))
            {
                ...
                Util.saveMetricsTEX(wr, metric_list, dataset, imbalancedData, isMeka, tableMetrics);
            ...
            else if(f1.getDescription().equals(".csv"))
            {
                ...
                Util.saveCSV(wr, metric_list, dataset, imbalancedData, isMeka, tableMetrics);
            ...
            else if (f1.getDescription().equals(".arff"))
            {
                ...
                Util.saveMetricsARFF(wr, metric_list, dataset, imbalancedData, isMeka, tableMetrics);
    ...
    private void button_saveActionPerformed_multi(java.awt.event.ActionEvent evt, JTable jtable) throws IOException
    {
        ...
        if(listMultiDatasets == null || listMultiDatasets.isEmpty() || datasetNames.isEmpty()) {
           JOptionPane.showMessageDialog(null, "You must load a dataset.", "Warning", JOptionPane.ERROR_MESSAGE);
           return; 
        }
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION)
        {
            ...
            if(f1.getDescription().equals(".txt"))
            {
                ...
                Util.saveMultiMetricsTXT(wr, metric_list, datasetNames, tableMetricsMulti);
            ...
            else if(f1.getDescription().equals(".tex"))
            {
                ...
                Util.saveMultiMetricsTEX(wr, metric_list, datasetNames, tableMetricsMulti);
            ...
            else if(f1.getDescription().equals(".csv"))
            {
                ...
                Util.saveMultiMetricsCSV(wr, metric_list, datasetNames, tableMetricsMulti);
            ...
            else if (f1.getDescription().equals(".arff"))
            {
                ...
                Util.saveMultiMetricsARFF(wr, metric_list, datasetNames, tableMetricsMulti);
    ...
    private void button_calculateActionPerformed_principal(java.awt.event.ActionEvent evt, JTable jtable)
    {
        ...
        label_frenquency = Util.orderImbalancedDataByFrequency(label_frenquency);// ordena de mayor a menor
        ...
        for(String metric : metric_list)
        {
           ...
           if((tableMetrics.get(metric) == null) || (tableMetrics.get(metric).equals("-"))){
               value = Util.getMetricValue(metric, dataset, isMeka);
                if(value.equals("-1.0") || value.equals("-1,0")){
                    value = Util.getImbalancedMetricValue(metric, dataset, imbalanced_data);
                } 	
    ...
    private void button_calculateActionPerformed_multi(java.awt.event.ActionEvent evt, JTable jtable)
    {
        ...
        if(listMultiDatasets == null || listMultiDatasets.size() < 1) {
            JOptionPane.showMessageDialog(null, "You must load a dataset.", "Warning", JOptionPane.ERROR_MESSAGE);
            return; 
        }
        ...
        for(String dataName : datasetNames){
            label_frenquency = Util.getFrequencyByLabel(listMultiDatasets.get(d));
            label_frenquency = Util.orderImbalancedDataByFrequency(label_frenquency);
            imbalanced_data = Util.getImbalancedByIRInterClass(listMultiDatasets.get(d), label_frenquency);
            ...
            for(String metric : metric_list)
            {
               ...
               if((tableMetricsMulti.get(dataName).get(metric) == null) || (tableMetricsMulti.get(dataName).get(metric).equals("-"))){
                   value = Util.getMetricValue(metric, listMultiDatasets.get(d), isMeka);
                    if(value.equals("-1.0") || value.equals("-1,0")){
                        value = Util.getImbalancedMetricValue(metric, listMultiDatasets.get(d), imbalanced_data);
                    } 	
    ...
    {
        ArrayList<String> metric_list = Util.getAllMetrics();
            ...
            {  
                try
                {
                    ...
                    if(comboBoxLabelsInformation.getSelectedIndex()==1){
                        //Labelsets tables
                        Util.saveLabelsetsFrequencyTableCSV(wr, jtable, labelsetStringsFreq);
                    }
                    else if(comboBoxLabelsInformation.getSelectedIndex()==6){
                        Util.saveTableIRLabelsetsCSV(wr, jtable, labelsetStringsIR);
                    }
                    else{
                        Util.saveTableCSV(wr, jtable);
                    }
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION)
        {
            ...
            if(f1.getDescription().equals(".csv"))
            {
                ...
                try
                {
                    ...
                    if(table.equals("ChiPhi")){
                        Util.saveChiPhiTableCSV(wr, chiPhiCoefficients, dataset.getLabelNames());
                    }
                    else if(table.equals("Coocurrence")){
                        Util.saveCoocurrenceTableCSV(wr, coocurrenceCoefficients, dataset.getLabelNames());
                    }
                    else if(table.equals("Heatmap")){
                        Util.saveHeatmapTableCSV(wr, heatmapCoefficients, dataset.getLabelNames());
                    }
                    else{
                        JOptionPane.showMessageDialog(null, "File not saved correctly.", "Error", JOptionPane.ERROR_MESSAGE); 
                    }
    ...
    private TableModel jchart_and_jtable_label_set_IR(JTable jtable, MultiLabelInstances dataset ,Statistics stat1, CategoryPlot cp ) throws Exception
    {
        ...
        labelsetStringsIR = new ArrayList<>(result.size());
        ...
        IRSortedLabelsets = new ImbalancedFeature[lista1.size()];
        ...
        while(!lista1.isEmpty())
        {
            ...
            labelsetStringsIR.add(temp.getName());
        ...
        //get mean
        double sum=0;
        for(int i=0; i<IRSortedLabelsets.length;i++)
        {
            sum+= IRSortedLabelsets[i].getIRIntraClass();
        }
          sum = sum/IRSortedLabelsets.length;
    ...
    private TableModel jchart_and_jtable_label_combination_freq(JTable jtable, MultiLabelInstances dataset ,Statistics stat1, CategoryPlot cp ) throws Exception
    {
        ...
        //Labelsets frequency
        HashMap<LabelSet,Integer> result = stat1.labelCombCount();
        labelsetStringsFreq = new ArrayList<>(result.size());
        ...
        sortedLabelsets = new ImbalancedFeature[lista1.size()];
        labelsetFrequency = new double[lista1.size()];
         
        while(!lista1.isEmpty())
        {
            temp = Util.getMax(lista1);
            sortedLabelsets[count-1]= temp;
            value = temp.getAppearances();            
            labelsetFrequency[count-1]= value;                    
            fila[0]=count;             
            freq =value*1.0/dataset.getNumInstances();             
            sum += freq;             
            String value_freq =Double.toString(freq);             
            fila[1]= value;             
            //fila[2]= Util.truncateValueAproxZero(value_freq,4);
            ...
            labelsetStringsFreq.add(temp.getName());
    ...
    private void jtable_coefficient_values(MultiLabelInstances dataset ,ArrayList<AttributesPair> lista_pares,String tipo_tabla)
    {
        double[][] pair_label_values;
         
        //coocurrence values table
        if(tipo_tabla.equals("coocurrence")) {
            pair_label_values =Util.getPairLabelValues(dataset, lista_pares);
        ...
        if(tipo_tabla.equals("coocurrence")){
            temp=jTableCoocurrence; 
            jpanel_temp=panelCoOcurrenceValues; 
            fixedTable_temp=fixedTableCoocurrence;
        }
        else {
            temp=jTableHeatmap;
    ...
    {
        chiPhiCoefficients = Util.getChiPhiCoefficients(dataset);
        ...
        {            
            data[num_fila]  = Util.getValuesByRow(num_fila, chiPhiCoefficients,dataset.getLabelNames()[num_fila]);
        }
    ...
    private TableModel jtable_label_graph(JTable jtable, MultiLabelInstances dataset )
    {
        ...
        for(int i=0;i<dataset.getNumLabels();i++)
        {
            ...
            fila[2]= Util.truncateValueAproxZero(truncate, 4);
    ...
    private TableModel jtable_ir_per_label_intra_class(JTable jtable)
    {
        ...
        Object[] fila = new Object[3];
        double[] visitados = new  double[imbalancedData.length];
        int cant_veces,id=1;
        String truncate;
        double current;
        ...
        {
            current= (imbalancedData[i].getIRIntraClass());
            truncate = Double.toString(current);
            
            cant_veces=  Util.getNumLabelsByIR(imbalancedData, visitados,current );

            if(cant_veces ==-1) continue;
            
            fila[0]=id;          
            fila[1]= cant_veces;
            fila[2]= Util.truncateValueAproxZero(truncate, 5);
    ...
    private TableModel jtable_ir_per_label_intra_class_only(JTable jtable)
    {
        ...
        for(int i=0; i<imbalancedData.length; i++)
        {                       
            truncate = Double.toString(imbalancedData[i].getIRIntraClass());
            
            fila[0]= imbalancedData[i].getName();          
            //fila[1]=Util.truncateValueAproxZero(truncate, 5);
            fila[1] = Util.getValueFormatted(truncate, 3);
    ...
    private TableModel jtable_ir_per_label_inter_class_only(JTable jtable)
    {
        ...
        IRInterClass= Util.getIRInterClassValues(labelFrequency); //calcula el ir inter class
        ...
        {                  
            ...
            fila[0]= labelFrequency[i].getName();          
            //fila[1]=Util.truncateValueAproxZero(truncate, 5);
            fila[1] = Util.getValueFormatted(truncate, 3);
    ...
    private TableModel jtable_ir_per_label_inter_class(JTable jtable)
    {
        ...
        Object[] fila = new Object[3];
        double[] visitados = new  double[labelFrequency.length];
        int cant_veces,id=1;
        String truncate;
        double current;
        
        idLabels_IRInterClass = new int [labelFrequency.length];
        id_IRInterClass = new double [labelFrequency.length];
        
        double[] IR_inter_class= Util.getIRInterClassValues(labelFrequency);
        
        for(int i=0; i< labelFrequency.length ; i++)
        {
            current= IR_inter_class[i];
            
            cant_veces=  Util.getNumLabelsByIR(IR_inter_class, visitados,current ); //calcula la cantidad de veces que aparece un mismo ir inter class

            if(cant_veces ==-1) continue;
            
            idLabels_IRInterClass[id-1]=cant_veces;
            id_IRInterClass[id-1]=current;
            visitados[id-1]=current;

            id++;
        }
        
        IRFreq = new ContainerIRInterClass(idLabels_IRInterClass, id_IRInterClass);
        IRFreq.ordena_IR_Mayor_a_Menor();
        
        idLabels_IRInterClass = IRFreq.Get_Id_x_Cant_veces();
        id_IRInterClass = IRFreq.Get_Id_x_IR();

        for(int i=0; i<id_IRInterClass.length; i++)
        {
            if(idLabels_IRInterClass[i] ==0) continue;
                        
            truncate = Double.toString(id_IRInterClass[i]);
            
            fila[0]= i+1;          
            fila[1]= idLabels_IRInterClass[i];
            fila[2]= Util.truncateValueAproxZero(truncate, 5);
    ...
    private TableModel jtable_imbalanced(JTable jtable, MultiLabelInstances dataset )
    {
        ...
        imbalancedData = Util.sortByIRIntraClass(imbalancedData);
    ...
    public void createTableMetricsPrincipal(JTable table,JPanel jpanel , Object rowData[][], int posx, int posy, int width,int height)
    {
        ...
        jtable = new JTable(jtable.getModel()){
            ...
            public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
                ...
                if (c instanceof JComponent) {
                    //if(column == 0){
                        JComponent jc = (JComponent) c;
                        jc.setToolTipText(Util.getMetricTooltip(getValueAt(row, 0).toString()));
                    //}
                }
                return c;
            }
    ...
    public void createTableMetricsMulti(JTable table,JPanel jpanel , Object rowData[][], int posx, int posy, int width,int height)
    {
    ...
    private double[][] getHeatMapCoefficients(){
        ...
        ImbalancedFeature [] label_frenquency = Util.getFrequencyByLabel(dataset);;
        double [] label_frenquency_values = Util.getLabelFrequency(label_frenquency);
